...
def build_kattotg_hierarchy(city: str, region: str, address: str):
    mongo = MongoService(CONFIG.MONGO.URI, CONFIG.MONGO.DB_NAME)

    if city == "Київ":
        return address, ("Київ", kyiv_kattotg), ("Київ", kyiv_kattotg)

    found_region = mongo.find_one(
        CONFIG.MONGO.KATOTTG_COLLECTION,
        {"name": region, "level2": None, "level3": None, "level4": None, "level5": None},
    )

    if not found_region:
        return address, None, None

    found_city = mongo.find_one(
        CONFIG.MONGO.KATOTTG_COLLECTION,
        {
            "level1": found_region["level1"],
            "level2": {"$ne": None},
            "level3": {"$ne": None},
            "level4": {"$ne": None},
            "level5": None,
    ...
    if not found_city:
        return address, None, (found_region.get("name", "n/a"), found_region.get("level1", "n/a"))

    return (
        address,
        (found_city.get("name", "n/a"), found_city.get("level4", "n/a")),
        (found_region.get("name", "n/a"), found_region.get("level1", "n/a")),
    )
...
def build_tender_kattotg_hierarchy(
    ...
    return (tender_address, (city["name"], city["level4"]), (region["name"], region["level1"]))
